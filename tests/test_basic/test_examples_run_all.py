...
def _configure_example_runtime(monkeypatch: pytest.MonkeyPatch, example_name: str) -> str:
...
def test_all_basic_examples_run(example_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    assert example_path.name in EXPECTED_OUTPUT_SNIPPETS

    output = io.StringIO()
    program_input = io.StringIO(_configure_example_runtime(monkeypatch, example_path.name))
