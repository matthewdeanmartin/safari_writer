...
WORD_TEXT = st.text(alphabet=string.ascii_letters + " ", min_size=1, max_size=20).filter(
...
def _alternating_case(text: str) -> str:
    return "".join(ch.upper() if index % 2 else ch.lower() for index, ch in enumerate(text))


@st.composite
def markdown_sections(draw: st.DrawFn) -> tuple[str, list[str]]:
    headings = draw(
        st.lists(WORD_TEXT, min_size=1, max_size=5, unique_by=lambda value: value.lower())
    )
    bodies = draw(
        st.lists(WORD_TEXT, min_size=len(headings), max_size=len(headings))
    )
...
def test_parse_document_assigns_sequential_chunk_ids(
    ...
    assert [chunk.heading for chunk in chunks] == [heading.strip() for heading in headings]
