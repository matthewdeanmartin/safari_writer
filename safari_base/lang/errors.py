...
class ParseError(DBaseError):
    ...
    def __init__(self, message: str, *, line_number: int | None = None, command_text: str = "") -> None:
        super().__init__(message, code="PARSE", line_number=line_number, command_text=command_text)


class NoTableError(DBaseError):
    """Raised when a command requires an open table but none is active."""

    def __init__(self, message: str = "No table is open in the current work area") -> None:
...
class UnsafeCommandError(DBaseError):
    ...
    def __init__(self, command: str) -> None:
        super().__init__(f"Unsafe command disabled: {command}. Enable unsafe=True to use.", code="UNSAFE")
