...
1. Treat each resulting block as a **topic chunk**.

1. For each chunk, extract:
...
1. Preserve original Markdown for display/debug/reference.
...
1. **ELIZA-style response composer**
1. **Conversation tree/state tracker**
1. **Distress and safety classifier**
1. **Retro UI renderer**
...
1. System normalizes input and checks for command shortcuts.

1. Safety/distress analysis runs.

1. Conversation tree updates with the new turn.

1. Retriever searches the Markdown topic chunks.

1. Response planner decides between:
...
1. Surface text is passed through synonym variation rules.
...
1. State clearly that it is not a psychiatrist or crisis professional.

1. Encourage the user to contact immediate real-world help.
...
1. Encourage the user not to stay alone if they are in immediate danger.

1. Avoid extensive back-and-forth analysis before giving this guidance.
...
1. Extract keywords and emotional markers.
1. Search chunk index.
1. Rank chunks.
1. Return top N chunks with scores.
1. If score threshold is not met, mark retrieval as weak.

### 14.2 Response planning order

1. Check crisis/self-harm rules.
1. Evaluate distress level.
1. Check for callback opportunity.
1. Evaluate retrieval confidence.
1. Build response.
1. Apply synonym variation.
1. Enforce tone and safety constraints.
...
1. The UI visibly resembles AtariWriter / Atari DOS conventions.
1. Menus and submenus are keyboard navigable.
1. The Chat subscreen is the primary interaction view.
1. The bottom command bar shows control-key shortcuts.
1. The top distress meter rises appropriately for distress or failed help.
1. The bot can vary wording without becoming incoherent.
1. The bot can reference something the user said earlier.
1. The bot is strongly apologetic about frustrating UX without claiming clinical expertise.
1. Suicide/self-harm language triggers real-help guidance and no fake-psychiatrist behavior.
