...
2. **Main Workspace (Rows 1–21):** The scrollable output area for commands like `LIST` or `DISPLAY`.
3. **The Dot Prompt (Row 22):** An active command line starting with a single period (`.`).
4. **Status Bar (Row 24):** A high-contrast bar divided into segments:
...
2. **Type:** (C, N, L, D, M).
3. **Width:** (Total characters).
4. **Decimals:** (For Numeric types).
