...
- **Workflow:**
  1. User presses `Z`.
  1. Prompt: `Archive Name?` (Defaults to `archive.zip` or `current_folder.zip`).
  1. Operation: Safari DOS creates a compressed ZIP file at the target location.
  1. Progress: Shows a progress bar or "Archiving: [filename]" status.
...
- **Workflow:**
  1. User presses `U`.
  1. Prompt: `Extract to [current_folder] (Y/N)?` or `Destination Path?`.
  1. Operation: Safari DOS extracts all contents, preserving the internal directory structure.
  1. Progress: Shows "Extracting: [filename]" status.
